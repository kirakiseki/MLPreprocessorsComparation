...
# 循环提交不同抽样比例任务
# -----------------------------
for RATE in 5 95 15 85 25 75 35 65 45 55;
do

for VOCAB_SIZE in 10000 50000 100000 200000;
do
    echo "重置Ray集群"
    bash $RESET_SCRIPT
    sleep 30
    ...
    RESULT_DIR="${CONTAINER_WORK_DIR}/run/${RATE}pct_${VOCAB_SIZE}vocab_${TIMESTAMP}"
    LOCAL_RESULT_DIR="${LOCAL_WORKING_DIR}/run/${RATE}pct_${VOCAB_SIZE}vocab_${TIMESTAMP}"
...
RUNTIME_ENV_JSON='{
    ...
    echo "=========================================="
    echo "提交 Ray 作业: 抽样比例 ${RATE}%, 词汇表大小 ${VOCAB_SIZE}"
...
ray job submit \
    ...
    -- python "${SCRIPT_NAME}" --sample_rate ${RATE} --vocab_size ${VOCAB_SIZE} --task_name "${TASK_NAME}"
EOF
    ...
    echo "重启Ray集群并备份"
    bash $RESET_SCRIPT -s "exp2_${RATE}pct_${VOCAB_SIZE}vocab"
