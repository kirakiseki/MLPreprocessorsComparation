...
研究针对分布式环境下的高维稀疏特征提取进行了对比实验。实验结果显示，Apache Spark在处理此类算子时表现出显著的稳定性和高效性。在相同的 5% 数据集负载下，Spark处理 200,000 维特征仅需 41 秒。与之形成鲜明对比的是，Ray Data 的执行延迟随特征维度的增加急剧恶化，在100,000维度以上的场景中，其执行时长超过1小时，性能衰减超过两个数量级。Ray 在全局字典构建阶段，随着特征维度增加，其分布式对象存储在处理大型元数据对象时产生了严重的序列化瓶颈Apache Spark 凭借其成熟的 BSP模型与深度优化的稀疏向量存储，在维度扩展与容量扩展上均展现了卓越的稳定性。相比之下，基于 Task 异步调度的Ray Data，在构建全局特征词典这一特定环节，受限于分布式对象存储的传输开销，在万维以上规模时出现了严重的性能退化。

![exp2-A](./assets/exp2-a.png)
![exp2-B](./assets/exp2-b.png)

图A和图B通过多维度的性能对比，直观揭示了Spark与Ray在处理高维稀疏特征时存在本质的架构差异。图A显示，Spark 在特征维度增加时表现出极强的鲁棒性，执行时长曲线近乎水平，证明其底层的稀疏向量表示和 Shuffle 优化对高维特征极不敏感；相比之下，Ray 的耗时随词汇量增加呈现出显著的指数级增长，尤其在 10 万维度以上陷入明显的性能瓶颈，反映了其 Task 调度模型在构建全局一致性字典时面临巨大的对象传输与序列化压力。图B进一步印证了Spark卓越的线性扩展能力，其实验曲线随数据量比例平稳上升，具有极高的可预测性；最震撼的对比在于，Ray 在仅 5% 数据负载下的延迟已远超 Spark在 95% 全量负载下的表现，这有力地证明了在处理需要强同步聚合的特征工程任务时，Spark的批处理架构具有压倒性的效率优势。

![exp2-C](./assets/exp2-c.png)

图C的Spark性能热力图则深度剖析了其作为工业级批处理引擎在处理“大宽表”任务时的稳定性优势。热力图的数值分布呈现出极强的规律性：执行时长主要受数据集比例的线性驱动，而受词汇量规模的影响极微。这种“双维度稳定性”充分体现了 Spark 对稀疏矩阵存储及局部聚合的深度优化，即在处理超高维度数据时，仅对非零项进行索引和计算，从而规避了维度激增带来的计算膨胀。热力图右下角的极值数据点显示，Spark 依然能稳定在 168 秒内完成任务，这为大规模机器学习预处理场景下的任务排期和资源评估提供了坚实的可靠性保障。
...
细粒度调度与对象传输成本：Ray 采用基于 Task 的并行模型，其优势在于流水线处理。然而，在处理 CountVectorizer 这种需要强一致性全局状态的任务时，Ray 的分布式对象存储面临巨大的序列化压力。 随着词汇量增加，跨节点传输大尺寸字典的开销成为了系统的主要瓶颈。
...
- 郭东旭（25%）：实验集群环境搭建、实验设计、运行实验、报告撰写、汇报视频录制
- 朱益辉（25%）：实验设计、实验代码编写、实验二数据分析、报告撰写
- 张京斌（25%）：实验设计、实验三数据分析、报告撰写、汇报视频录制
- 杜茗天（25%）：实验集群环境搭建、实验代码编写、实验一数据分析、报告撰写
